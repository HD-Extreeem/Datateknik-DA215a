...
uint8_t input_int(char *p_msg, uint16_t *p_int_nr)
{
	...
	while (1) {
		...
		if (key == '#') {					// confirmation of the number?
			...
			if (length > 0) {
				lcd_write(INSTRUCTION, 0x04);  //jumps back to writtn key
				lcd_write(DATA, ' ');			// overwrites a empty space
				lcd_write(INSTRUCTION, 0x06);	// jumps to the front again
				delay_ms(255);
				length --;
			}
		...
		do {								// read keyboard....
			key = numkey_read();		//checks if key is the same then it doesnt write anything
			delay_ms(10);			
		} while (key == lastkey);
